/**
 * classify.js — Motor que aplica las reglas 3.2, 3.3 y 3.4 a texto libre.
 *
...
import { DELEGACION, CATEGORIAS, DECISIONES_PROPIAS, PERSONAS, CAT } from './rules.js';
...
function matchPista(norm, pista) {
  return pistaToRegex(pista).test(norm) ? pista.length : 0;
}

function mejorMatch(norm, items) {
  let best = null;
 ...
 */
export function clasificarFragmento(texto) {
 ...
 */
export function clasificarCheckin(texto, horasDia = 8) {
 ...
 */
export function evaluarPrioridad(texto) {
 ...
 */
export function detectarResponsables(texto) {
  const norm = normalizar(texto);
  const encontrados = new Set();
  for (const [id, p] of Object.entries(PERSONAS)) {
    for (const alias of p.alias) {
      if (matchPista(norm, normalizar(alias))) { encontrados.add(id); break; }
    }
  }
  return [...encontrados];
}

export function validarResponsableUnico(texto) {
  const t = (texto || '').trim();
...
export function totalesSemana(checkins) {
  const porCategoria = {};
  const diasConCategoria = {};
  let total = 0;
  const dias = new Set();
  for (const ck of checkins) {
    if (!ck || !ck.segmentos?.length) continue;
    dias.add(ck.fecha);
  ...
  return { porCategoria, diasConCategoria, total, diasRegistrados: dias.size };
}

/** Tareas de la tabla 3.2 que hiciste vos, agrupadas por dueño real. */
export function fugasDelegacion(checkins) {
  const porDueno = {};
  for (const ck of checkins) {
    for (const s of ck?.segmentos || []) {
      if (!s.delegacion) continue;
      const k = s.delegacion.dueno;
      porDueno[k] = porDueno[k] || { dueno: k, nombre: s.delegacion.duenoNombre, horas: 0, tareas: new Set(), veces: 0 };
